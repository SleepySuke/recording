...
- [x] 1. 实现 Dockerfile：多阶段构建（builder 缓存 go mod → 最小运行镜像），非 root 用户，EXPOSE 8080。
- [x] 2. 实现 Makefile 与 compose 最终版（app 服务 `depends_on: db: condition: service_healthy`；两个数据卷；`./logs:/app/logs` 绑定；app healthcheck 打 `/readyz`）。
- [x] 3. 验证清单（干净环境路径，逐项打勾）：
  - [x] `make check`：逐项输出 OK/缺失，单独可执行。
  - [x] `make setup && make start`：从零到 `/readyz` 200；迁移自动执行，无需手动建表。
  - [x] `make start` 幂等：镜像已存在直接复用；`make build` 后才重建。
  - [x] `make dev`：本地起 db + `go run`，日志到终端。
  - [x] `make down && make start`：容器重建后录音文件与 MySQL 数据仍在（卷持久化）。
  - [x] `make logs` 跟踪；宿主机 `./logs/app.jsonl` 可写。
  - [x] 迁移失败注入（改坏 DSN）→ 应用不就绪（健康检查不过）。
  - [x] `make lint` 通过。
- [ ] 4. 提交：`build: add makefile, dockerfile and compose stack`。

## 完成标准

- [x] 新环境（清掉镜像/卷）两步启动至就绪，全程无手工 SQL。
- [x] README「运行方式」表格中的每条命令都真实存在且行为一致（README 正文回写留 T14 统一做）。
