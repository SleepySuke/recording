...
- [ ] 1. 写失败测试（`tests/integration/retry_test.go`）：
...
- [ ] 2. 运行确认失败：`TEST_MYSQL_DSN=… go test ./tests/integration/ -run 'TestIT06|TestIT07|TestRetry' -v`。
...
- [ ] IT-06/IT-07 全绿；重复请求不新增任务（task_id 复用）。
- [ ] 手动：失败任务 retry 后从头执行成功（attempt=2）。
