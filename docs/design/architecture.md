...
不纳入首轮：鉴权、前端、对象存储、高并发优化、自动重试、SSE、公网部署。上传幂等（哈希去重）已完成设计预留（详设 §5.1），按开发计划作为可选加分项实现。真实 ASR 与前端为后续扩展，顺序见开发计划。
...
**失败与重试**：任一阶段失败写入 failed 后，`POST /v1/tasks/{task_id}/retry` 以条件更新（仅 failed）回到 pending，attempt 加一、清空上轮产物，从头重新执行；同一轮重复重试返回 409。完整语义见详设 §8。
...
| content_hash，CHAR(64) 可空 | 流式写入时计算的 SHA-256；上传去重/幂等的数据基础，启用幂等时补唯一索引 |
