...
状态：实施前设计，不表示业务代码已完成。总体结构、数据模型与状态机见 [总体架构](architecture.md)，开发任务见 [任务文档](../plan/tasks/T01-bootstrap.md)（T01～T14 顺序执行）。本文档是详细设计的唯一权威版本，合并了此前的数据库选型、DDD 分层、任务生命周期日志与错误码文档。
...
- **去重与上传幂等（可选增强，尚未实现）**：流式写入时同步计算 SHA-256（无额外读盘开销），存入 `recordings.content_hash`；完整的判定语义、哈希锁表、上传/删除并发数据流、文件补偿、端口和 HTTP 契约以[上传幂等设计](upload-idempotency.md)为唯一权威。首轮仅计算并存储哈希；命中复用由 T15 实现。
