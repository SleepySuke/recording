...
状态：T01～T16 已实施；自动重试与 SSE 的新增权威约定见 [自动重试与摘要事件流设计](retry-and-summary-stream.md)。总体结构、数据模型与状态机见 [总体架构](architecture.md)。本文档保留既有详细设计约定。
...
- **去重与上传幂等（已实现）**：流式写入时同步计算 SHA-256（无额外读盘开销），存入 `recordings.content_hash`；完整的判定语义、哈希锁表、上传/删除并发数据流、文件补偿、端口和 HTTP 契约以[上传幂等设计](upload-idempotency.md)为唯一权威。命中未删除录音时复用其任务；T15 已实现并由集成与 E2E 覆盖。
...
必须处理：网络错误、上下文超时、非 2xx、空响应、外层结构异常、JSON 无法解析、字段类型错误。严格验证 JSON 对象**恰好**包含 summary、key_points、todos：summary 为非空字符串，key_points / todos 为存在的字符串数组且元素非空字符串，空数组允许；拒绝 null、缺字段、额外字段、混入额外文本、多个 JSON 值；首轮直接拒绝 Markdown 围栏，不做"修复输出"。供应商有结构化输出能力可启用，本地验证仍必须存在。

自动重试的轮次、退避、费用边界及最终手动重试语义见 [自动重试与摘要事件流设计](retry-and-summary-stream.md)；Mock 摘要只用于自动化测试，真实渠道用于人工验收。

## 10. 验证要点

测试分层、替身设施、全部用例（单元 / 集成 / E2E）与 E2E 的"预期-真实"比对机制统一见 [测试设计](test-design.md)；任务排期与完成状态见[任务文档](../plan/tasks/T01-bootstrap.md)（同目录 T01～T14）。技术文档只保留两条设计层约束：
