...
// POST /v1/tasks/:id/retry（T08，详设 §8.1）；删除接口随 T09 挂载。
func New(logger *slog.Logger, upload *handler.UploadHandler, query *handler.QueryHandler, retry *handler.RetryHandler) *gin.Engine {
	r := gin.New()
	r.Use(
